...
def training(config, dataset=None, checkpoint_dir=None, data_dir=None):
    ...
    trainout = {}
    for documentid in trainoutpre.keys():
        author = documentid.split('_')[0]
        if trainout.get(author) is not None:
            pass
        else:
            trainout.update({author: {}})
        doc_id = documentid.split('_')[1]
        trainout[author].update({doc_id: {}})
        trainout[author][doc_id].update({'label': float(trainoutpre[documentid]['label'])})
        trainout[author][doc_id].update({'predict': int(trainoutpre[documentid]['predict'])})
        trainout[author][doc_id].update({'prob': trainoutpre[documentid]['prob']})

    authors = trainout.keys()
    correct = 0
    trainoutfinal = {}
    total = 0
    for author in authors:
        n_doc = len(trainout[author])
        lebel = int(trainout[author][list(trainout[author].keys())[0]]['label'])
        vote = 0
        for j in list(trainout[author].keys()):
            vote = vote + trainout[author][j]['predict']
        vote = vote / n_doc
        final_decision = np.round(vote)
        correct += (final_decision == lebel).sum()
        total = total + 1
        trainoutfinal.update({author: {}})
        trainoutfinal[author].update({'predict': int(final_decision)})
        trainoutfinal[author].update({'label': int(lebel)})

    trainallscore = correct / total
    with open(os.path.join(resultsdir, 'trainsetresults_' + str(trainallscore)[:6] + ".json"), 'w', encoding='utf-8') as f:
        json.dump(trainout, f, ensure_ascii=False, indent=4)
    with open(os.path.join(resultsdir, 'trainsetfinalresults_' + str(trainallscore)[:6] + ".json"), 'w', encoding='utf-8') as f:
        json.dump(trainoutfinal, f, ensure_ascii=False, indent=4)
    ...
    with open(os.path.join(resultsdir, 'testsetfinalresults_' + str(allscore)[:6] + ".json"), 'w',
              encoding='utf-8') as f:
